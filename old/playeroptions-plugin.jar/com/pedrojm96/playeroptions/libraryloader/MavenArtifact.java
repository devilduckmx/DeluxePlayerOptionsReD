...
public class MavenArtifact {
    ...
    public URL getUrl() throws MalformedURLException {
        ...
        if (!repo.endsWith("/")) {
            repo = String.valueOf(String.valueOf(repo)) + "/";
        }
        repo = String.valueOf(String.valueOf(repo)) + "%s/%s/%s/%s-%s.jar";
        String url = String.format(repo, this.groupId.replace(".", "/"), this.artifactId, this.version, this.artifactId, this.version);
        return new URL(url);
    }

    public String getPath() {
        String path = String.valueOf(this.groupId.replace('.', '/')) + '/' + this.artifactId + '/' + this.version + '/' + this.artifactId + '-' + this.version + ".jar";
        return path;
    }
